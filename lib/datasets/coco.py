...
class coco(imdb):
    def __init__(self, image_set, year):
        ...
        self._COCO = COCO(self._get_ann_file())                  # 此处使用了 self._image_set 以确定是训练集还是测试集
        cats = self._COCO.loadCats(self._COCO.getCatIds())                          # 加载类别标签
        self._classes = tuple(['__background__'] + [c['name'] for c in cats])       
        self._class_to_ind = dict(zip(self.classes, xrange(self.num_classes)))      # 类别及其对应编号
        self._class_to_coco_cat_id = dict(zip([c['name'] for c in cats],            # 类别名称及对应cat_id
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_coco_annotation(index)           # 读取每一张图片上的标注信息
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        ...
        # 仅仅用于非 test 数据集的测试（比如验证集val）
        # 输入：1. 预测结果的json文件(res_file) 2. 输出目录
