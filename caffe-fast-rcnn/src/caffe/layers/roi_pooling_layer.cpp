...
void ROIPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->cpu_data();                   // bottom[0] 中存放的是特征图
  const Dtype* bottom_rois = bottom[1]->cpu_data();                   // bottom[1] 中存放的是roi
  // Number of ROIs
  ...
  for (int n = 0; n < num_rois; ++n) {
    ...
    int roi_height = max(roi_end_h - roi_start_h + 1, 1);              //至少要为1
    ...
    for (int c = 0; c < channels_; ++c) {
      for (int ph = 0; ph < pooled_height_; ++ph) {
        for (int pw = 0; pw < pooled_width_; ++pw) {
          ...
          hstart = min(max(hstart + roi_start_h, 0), height_);           //避免越界
