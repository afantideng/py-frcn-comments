...
void SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  caffe_gpu_sub(
      count,                        
      bottom[0]->gpu_data(),        //预测坐标
      bottom[1]->gpu_data(),        //target坐标
  ...
  if (has_weights_) {
    ...
    caffe_gpu_mul(
        count,
        bottom[2]->gpu_data(),      //inside weights 有物体时为1，否则为0
        diff_.gpu_data(),
        diff_.mutable_gpu_data());  
  }

  // d := SmoothL1(w_in * (b0 - b1))
  ...
  if (has_weights_) {
    ...
    caffe_gpu_mul(
        count,
        bottom[3]->gpu_data(),        // outside weights 损失函数最左边的归一化系数
        errors_.gpu_data(),
        errors_.mutable_gpu_data());  // d := w_out * SmoothL1(w_in * (b0 - b1))
  }

  Dtype loss;
  // 将所有的像素点累加得到一个值，就是loss
...
void SmoothL1LossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      // bottom[i] = alpha * diff_
      caffe_gpu_axpby(                     //y = a*x + b*y
          count,                           // count
          alpha,                           // alpha
          diff_.gpu_data(),                // x
          Dtype(0),                        // beta
